...
- **기타 조건을 사용자 화면에도 노출할지 결정**: `otherRequirements`(177개)는 지금 관리자만 본다. 유닛의 44%에 있는 정보라 사용자에게도 "이런 추가 조건이 있으니 원문을 확인하세요"로 보여주는 게 유용할 수 있다.
- **PDF 없는 공고 3건 처리 방침**: 상세 페이지에 첨부가 아예 없어 자동 추출이 불가능하다. 관리자가 원문 사이트에서 직접 확인해 수동 입력하거나, 그런 공고는 "확인 필요" 상태로 두고 원문 링크만 강조하는 방식을 정해야 한다.
...
- **관리자 권한 분리**: `/admin/*`이 이제 "로그인한 사람"은 통과시킨다 — 아직 관리자/일반 사용자 구분이 없어서 **가입한 누구나 관리자 화면에 들어갈 수 있다**. 배포 전 반드시 role 기반 검사 필요(예: `user_profiles.role` 컬럼 + proxy에서 확인).
