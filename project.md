...
- **PDF→LLM 조건 추출 파이프라인이 실제로 동작한다**(9번 단계) — `npm run extract:conditions`로 공고 PDF를 Gemini에게 읽혀 자격요건 초안을 뽑고, `/admin/ai-drafts`에서 검수·반영까지 엔드투엔드로 검증 완료. **나머지 126건 배치 추출이 지금 진행 중**(Gemini 무료 티어 분당 20회 한도에 걸려 재작업, 10번 단계 참고).
...
- **배치 추출 결과 확인 + 관리자 검수**: rate limit 수정 후 재실행한 117건 배치가 끝나면 결과(성공/실패/no_pdf 건수) 확인하고, `/admin/ai-drafts`에서 하나씩 검수·반영. 지금까지 3건은 파이프라인 검증용으로 이미 처리됨(그 중 1건은 실제로 ready 전환까지 완료).
- **AI 추출 품질을 여러 건으로 더 확인**: 표가 복잡하거나 이미지로만 된 PDF(스캔본 등)에서는 품질이 떨어질 수 있어 더 넓은 샘플로 확인 필요.
- **통근 추천 기능 이어서 개발**(11번 단계): 지금은 타입만 있음. 온보딩 UI, 카카오맵 길찾기 연동, 경쟁률 데이터 조사, 추천 점수·한줄평 로직이 남음. 회원 인증이 선행되어야 함.
