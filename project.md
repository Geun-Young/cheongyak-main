...
- **인증·권한**: 로그인 3종(12번) + 관리자 권한 분리(16번) 완료. `/admin/*`은 `admins` 테이블에 등록된 계정만 접근 가능하고, 관리자 지정은 `npm run grant-admin -- 이메일`로만 한다.
...
### 17. 카카오 로그인이 KOE205로 막힘 — 버튼을 감추고 구글·이메일로 진행

Supabase 대시보드에서 google·kakao·email provider를 모두 켜고 카카오 디벨로퍼스 앱 설정(REST API 키, Client Secret,
Redirect URI, 플랫폼 등록)까지 마쳤는데도 카카오 로그인만 **KOE205**로 실패했다.

**원인**: Supabase가 카카오 인가 요청에 `account_email` scope를 **강제로** 넣는다. 그런데 카카오의 "카카오계정(이메일)"
동의항목은 **비즈앱으로 전환해야** 열리는 항목이라, 개인 개발자 앱에서는 앱에 없는 동의항목을 요구하는 꼴이 되어 거부된다.

**시도했지만 안 된 것**: `signInWithOAuth`에 `scopes: "profile_nickname"`을 줘봤다. 실제로 나가는 URL을 확인해 보니
`account_email+profile_image+profile_nickname+profile_nickname` — Supabase의 기본 scope를 **덮어쓰는 게 아니라 뒤에 덧붙이기만** 한다.
즉 앱 코드로는 막을 방법이 없다. 이 시도는 되돌리고, 왜 안 되는지를 `lib/auth.ts` 주석으로 남겼다(같은 걸 또 시도하지 않도록).

**선택**: 비즈앱 전환은 본인인증 절차가 필요해 지금 당장 하기엔 부담이라, **카카오 버튼을 감추고 구글·이메일로 먼저 간다**.
누르면 실패하는 버튼을 두느니 안 보이는 게 낫다는 판단.

- `SocialLoginButtons`에 `NEXT_PUBLIC_ENABLE_KAKAO_LOGIN === "true"`일 때만 카카오 버튼을 렌더한다. 기본값은 숨김이고,
  비즈앱 전환이 끝나면 환경변수 한 줄만 넣으면 코드 수정 없이 되살아난다. 버튼 코드·아이콘·`kakao` 버튼 variant는 그대로 둔다.
- 랜딩 페이지 CTA "카카오로 3초 만에 시작" → **"3초 만에 시작하기"**. 카카오가 안 보이는데 카카오로 시작하라고 하면
  약속과 화면이 어긋난다. provider 중립 문구라 나중에 카카오가 돌아와도 그대로 맞다.
- `.env.local.example`에 플래그와 그 이유를 적어뒀다.

**검증**: `npm run lint`, `next build`, `tsc --noEmit` 모두 통과.
...
- **카카오 로그인 되살리기**: 카카오 디벨로퍼스에서 **비즈앱 전환**(개인 개발자도 본인인증으로 가능, 사업자등록번호 불필요)을 해야 `account_email` 동의항목이 열린다. 전환 후 `.env.local`에 `NEXT_PUBLIC_ENABLE_KAKAO_LOGIN=true`를 넣으면 버튼이 다시 나온다(17번). 전환 없이 쓰려면 이메일을 안 받는 방법이 필요한데, Supabase가 scope를 강제로 붙여서 앱 코드로는 못 막는다.
- **이메일 확인(Confirm email) 끄기**: 사용자가 "이메일 확인 필수 아니다"라고 했지만 이건 대시보드에서만 바꿀 수 있다(Authentication > Providers > Email > Confirm email). 마지막 확인 시점에 여전히 켜져 있었다.
