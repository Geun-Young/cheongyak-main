import type { Announcement, Condition, Field, Operator, FactValue, ScoreRule, SupplyUnit, Tier } from "../types";
...
let unitSeq = 0;
/**
 * 공급유닛 헬퍼. id는 호출부에서 안 넘기면 자동 채번하되,
 * 공고 id에서 파생시키고 싶으면 announcementId를 넘긴다(마이그레이션 편의용, 최종 id는 아래에서 재부여).
 */
function unit(partial: Omit<SupplyUnit, "id" | "summary" | "tiers" | "scoreRules"> & {
  id?: string;
  summary?: string[];
  tiers?: Tier[];
...
/** supplyUnits의 id를 "${announcementId}-u1" 형식으로 재부여하고 units 총합을 계산해 붙인다 */
function withUnits(
  id: string,
  base: Omit<Announcement, "id" | "supplyUnits" | "units">,
  units: SupplyUnit[],
): Announcement {
  const supplyUnits = units.map((u, i) => ({ ...u, id: `${id}-u${i + 1}` }));
  return {
    id,
    ...base,
...
export const ANNOUNCEMENTS: Announcement[] = [
  withUnits(
    ...
    [
      unit({
        ...
        rankingMethod: "순위+가점",
        unitsCount: 180,
        address: "서울 강동구 고덕강일로 100",
        lat: 37.5665,
        lng: 127.1584,
        moveIn: "예비입주자 순번에 따라 수시",
        rentNote: "보증금 2,800만 원 / 월 18만 원",
      ...
      unit({
        ...
        rankingMethod: "순위+가점",
        unitsCount: 140,
        address: "서울 강동구 강일운로 50",
        lat: 37.5637,
        lng: 127.1745,
        moveIn: "예비입주자 순번에 따라 수시",
        rentNote: "보증금 4,100만 원 / 월 27만 원",
        summary: ["46㎡는 청약통장 6회 이상 납입한 경우에만 신청할 수 있어요."],
        eligibility: [noHouse(), income(100), assets(33_700), car(3_803), payCount(6)],
        tiers: [
          tier(1, "서울 거주", [region("서울")]),
          tier(2, "수도권 거주", [regionIn(CAPITAL_AREA, "수도권 거주")]),
          tier(3, "그 외 지역", []),
  ...
  withUnits(
    ...
    [
      unit({
        ...
        rankingMethod: "순위+가점",
        unitsCount: 90,
        address: "부산 강서구 명지국제도시7로 20",
        lat: 35.0956,
        lng: 128.9057,
        moveIn: "2027년 6월",
        rentNote: "보증금 1,900만 원 / 월 14만 원",
      ...
      unit({
        ...
        lng: 128.9057,
        moveIn: "2027년 6월",
        rentNote: "보증금 3,300만 원 / 월 24만 원",
        summary: ["51㎡는 청약통장 12회 이상 납입한 경우에만 신청할 수 있어요."],
        eligibility: [noHouse(), income(70), assets(33_700), car(3_803), region("부산"), payCount(12)],
        tiers: [tier(1, "부산 거주", [region("부산")])],
        scoreRules: nationalRentalScore,
      }),
    ],
  ...
  withUnits(
    ...
    [
      unit({
        name: "전용 59·74·84㎡ 일반공급",
        housingType: "공공분양",
        rankingMethod: "가점제",
        unitsCount: 610,
        address: "경기 고양시 덕양구 창릉동",
        lat: 37.6273,
        lng: 126.8735,
        moveIn: "2029년 상반기",
  ...
  withUnits(
    ...
    [
      unit({
        ...
        rankingMethod: "순위+가점",
        unitsCount: 150,
        address: "경기 과천시 지식정보로 90",
        lat: 37.4229,
        lng: 127.0224,
        moveIn: "2027년 9월",
        rentNote: "보증금 6,800만 원 / 월 32만 원",
